// Harvest job IDs from JobRight intern minisites (the grid behind intern-list.com).
import { chromium } from 'playwright-core';
import fs from 'fs';
const CATS = (process.env.CATS || 'swe').split(',');
const b = await chromium.connectOverCDP('http://localhost:9222');
const c = b.contexts()[0];
const out = new Map();
const p = await c.newPage();
for (const cat of CATS) {
  const url = `https://jobright.ai/minisites-jobs/intern/us/${cat}`;
  await p.goto(url, { waitUntil: 'domcontentloaded', timeout: 35000 }).catch(() => {});
  await p.waitForTimeout(6000);
  await p.evaluate(() => { const t = document.getElementById('___reactour'); if (t) t.remove(); });
  let stale = 0, before = 0;
  for (let r = 0; r < 60 && stale < 5; r++) {
    const found = await p.evaluate(() => {
      const res = [];
      document.querySelectorAll('a[href*="jobs/info"]').forEach(a => {
        const m = (a.getAttribute('href') || '').match(/jobs\/info\/([0-9a-f]{24})/);
        if (!m) return;
        const row = a.closest('[class*=row],[role=row]') || a.parentElement?.parentElement;
        res.push({ id: m[1], txt: ((row?.innerText) || a.innerText || '').replace(/\s+/g, ' ').trim().slice(0, 120) });
      });
      return res;
    }).catch(() => []);
    found.forEach(f => { if (!out.has(f.id) || out.get(f.id).length < f.txt.length) out.set(f.id, f.txt); });
    stale = out.size === before ? stale + 1 : 0;
    before = out.size;
    // the grid is its own scroll container
    await p.evaluate(() => {
      window.scrollBy(0, 4000);
      [...document.querySelectorAll('div')].filter(e => e.scrollHeight > e.clientHeight + 100 && e.clientHeight > 250)
        .forEach(e => { e.scrollTop += 4000; });
    });
    await p.waitForTimeout(1400);
  }
  console.log(`${cat}: ${out.size} unique so far`);
}
await p.close();
const arr = [...out.entries()].map(([id, txt]) => ({ id, txt }));
fs.writeFileSync('ms_jobs.json', JSON.stringify(arr, null, 1));
console.log(`TOTAL harvested: ${arr.length}`);
arr.slice(0, 5).forEach((j, i) => console.log(` ${i + 1}. ${j.id} ${j.txt.slice(0, 70)}`));
await b.close();
